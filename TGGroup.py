...
class Group:
    ...
    def split(self, n, total_split=False):
        ...
        if total_split:
            mod = len(members)//n
            self.members = [members[i*mod:(i+1)*mod] if (i+2)*mod <=
                            len(members) else members[i*mod:] for i in range(n)]
        else:
            self.members = [
                members[i:i+n] if (i+n) <= len(members) else members[i:] for i in range(0, len(members), n)]
        self.save()
        return self.members
    ...
    def todict(self):
        ...
        result = {attr: getattr(self, attr)
                  for attr in ["id", "name", "access_hash"]}
    ...
    def chooser_config(self):
        ...
        result.update({"memcount": {i: len(k)
    ...
    def __str__(self):
        print_attr = [attr+"="+str(atval)
                      for attr, atval in self.todict().items()]
...
class GroupStore:
    ...
    def load(self, client_phone):
        self.group_store = []
        group_path = os.path.join("data", "groups", "*"+client_phone[1:]+".json")
    ...
    def get(self, **kwargs):
        for i, group in enumerate(self.group_store):
            flag = all(str(kwargs[attr]) == str(
                getattr(group, attr)) for attr in kwargs)
            if flag:
                return group
        return None
    ...
    def delete_all(self):
        [group.delete() for group in self.group_store]
        self.group_store = []
